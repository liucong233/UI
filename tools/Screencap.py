...
def GetScreenbyADBCap(starttime, devices, action):
    ...
    png = screenpath + "\\" + time.strftime('%Y%m%d_%H%M%S',
                                            time.localtime(starttime)) + nickname + "_" + action + ".png"
    print("png=", png)
...
def GetScreenbyMiniCap(starttime, devices, action, ssFlipped=False):
    ...
    screen = adb + " -s {} shell \" LD_LIBRARY_PATH=/data/local/tmp /data/local/tmp/minicap -P {}@{}/0 -s > /sdcard/screencap.png\"".format(
        devices, size, size)
    print(screen)
    os.popen(screen)
    time.sleep(0.5)
