...
class ReportTestResult(unittest.TestResult):
    ...
    def __init__(self, suite, stream=sys.stdout):
        ...
        self.default_brand_name = 'oneplus6'
        self.default_version_name = '7.0.0'
        self.default_time_name = '2019-09-26 09:02:32'
        self.default_branch_name = '0927'
    ...
    def stopTestRun(self, title=None, brand1=None, version1=None, jenkins_time=None, jenkins_branch=None) -> dict:
        """
            所有测试执行完成后, 执行该方法
        :param title:
        :return:
        ...
        self.fields['testbrand'] = brand1 if brand1 else self.default_brand_name
        self.fields['testversion'] = version1 if version1 else self.default_version_name
        self.fields['testjenkins_time'] = jenkins_time if jenkins_time else self.default_time_name
        self.fields['testjenkins_branch'] = jenkins_branch if jenkins_branch else self.default_branch_name
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def __init__(self, suites):
        ...
        self.brand1 = 'onepluse6'
        self.version1 = '7.0.0'
        self.jenkins_time = '2019-09-26 09:02:32'
        self.jenkins_branch = '0927'

    def report(self, description, brand, version, jenkins_branch, jenkins_time, filename: str = None, report_dir='.',
               log_path=None, theme='theme_default', report_flag=True):
        """
            生成测试报告,并放在当前运行路径下
        ...
        self.stopTestRun(self.title, self.brand1, self.version1, self.jenkins_time, self.jenkins_branch)
        if not report_flag:
            self.output_report(theme)
            text = '测试已全部完成, 可打开 {} 查看报告'.format(os.path.join(self.report_dir, self.filename))
            logger.info(text)
        else:
